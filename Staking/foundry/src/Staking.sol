...
contract Staking {
    ...
    function stakeEther(uint256 _numDays) external payable {
        ...
        Position memory position = Position({
            positionId: s_currentPositionId,
            walletAddress: msg.sender,
            createdDate: block.timestamp,
            unlockDate: block.timestamp + (_numDays * 1 days),
            percentInterest: s_tiers[_numDays],
            weiStaked: msg.value,
            weiInterest: InterestRates.calculateInterest(s_tiers[_numDays], _numDays, msg.value),
            open: true
        });
